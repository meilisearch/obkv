fn varint_length_packed(data: &[u8]) -> Option<u32> {
    let mut i = 0;
    for _ in 0..data.len() {
        if (*data.get(i)? & 0x80) == 0 {
            break;
        }
        i += 1;
    }
...
pub fn varint_decode32(data: &[u8], value: &mut u32) -> Option<usize> {
    ...
    if len > 1 {
        ...
        if len > 2 {
            val |= ((*data.get(2)? & 0x7f) as u32) << 14;
            if len > 3 {
                val |= ((*data.get(3)? & 0x7f) as u32) << 21;
                if len > 4 {
                    val |= (*data.get(4)? as u32) << 28;
                }
            }
        }
    }
...
mod tests {
    ...
    quickcheck! {
        fn qc_codec_u32(num: u32) -> bool {
            ...
            varint_decode32(buf, &mut val).unwrap();
